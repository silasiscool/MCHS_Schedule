...
// set date box
dateBox.innerHTML = ['Sunday','Monday','Tuesday','Wednesday','Thursday','Friday','Saturday'][new Date().getDay()]+', '+monthDayYear(new Date());
...
config_file.then((config) => {
  ...
  calendarBoxes.forEach((calendarBox, i) => { // do following for each box
    ...
    if (boxDaySchedule.tag === '') {
      calendarBox.innerHTML = boxDate.getDate();
    } else {
      calendarBox.innerHTML = boxDaySchedule.tag;
    };
  });
  ...
  // set dayTypeBox
  dayTypeBox.innerHTML = currentDaySchedule.display_name;

  // create function to update the current boxes
  function updateCurrent() {
    // store current time peices
    currentDate = new Date(new Date().setSeconds(new Date().getSeconds()));
    ...
    if (periodIndex === undefined) { // if no school day /  after school
      ...
      let extraDays = 1
      while (true) {
        nextMondayDate = new Date(new Date().setDate(nextDate.getDate()-nextDate.getDay()+1));
      ...
      while (true) {
        previousMondayDate = new Date(new Date().setDate(previousDate.getDate()-previousDate.getDay()+1));
